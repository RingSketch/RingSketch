...
void testCaseParallel(std::ofstream &outFile, int memory_base) {
    ...
    for (int i = 16; i >= 4; i--) {
        for(int counterNum = 4; counterNum <= 16; counterNum += 4) {
                tasks.emplace_back(i, counterNum, 16, 8, "group1");
                order_counter++;
            }
        }

    for (int i = 16; i >= 4; i--) {
        for(int counterSize = 4; counterSize <= 20; counterSize += 4) {
                tasks.emplace_back(i, 4, counterSize, 4, "group1");
                order_counter++;
            }
        }
...
int main(int argc, char* argv[]) {
    ...
    dataset = read_data("../datasets/caida.dat", 1 << 20, &datasetLength).first;
    std::cout << "Dataset length: " << datasetLength << std::endl;

    int memory_base = 8000;

    std::string output_filename = "../results/parameter.csv";
