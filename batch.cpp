...
std::string file[1] = {"caida.dat"};

int main(int argc,char* argv[]) {
    srand(233);

    dataset = read_data((folder + file[0]).c_str(), 1 << 22, &datasetLength);
    ...
    freopen("../results/result_batch_different_algo.csv", "w", stdout);
    printf("name,windowSize,threshold,memory,counterNum,counterSize,rr,pr,f1\n");
    testBatch(4, 4);
    
    freopen("../results/result_batch_different_counterNum.csv", "w", stdout);
    printf("name,windowSize,threshold,memory,counterNum,counterSize,rr,pr,f1\n");
    testBatch(10, 1);
    return 0;
}
